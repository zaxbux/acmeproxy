...
// Present gets the TXT record and calls CreateRecord to create it.
func (d *DNSProvider) Present(domain, token, keyAuth string) error {
	fqdn, value := dns01.GetRecord(domain, keyAuth)
	return d.CreateRecord(fqdn, token, value)
}

// CreateRecord creats a TXT record to fulfill the dns-01 challenge
func (d *DNSProvider) CreateRecord(fqdn, token, value string) error {
...
func (d *DNSProvider) CleanUp(domain, token, keyAuth string) error {
	fqdn, value := dns01.GetRecord(domain, keyAuth)
	return d.RemoveRecord(fqdn, token, value)
}

// RemoveRecord removes the TXT record matching the specified parameters.
func (d *DNSProvider) RemoveRecord(fqdn, token, value string) error {
	authZone, err := dns01.FindZoneByFqdn(fqdn)
	if err != nil {
		return fmt.Errorf("cloudflare: %w", err)
	}
